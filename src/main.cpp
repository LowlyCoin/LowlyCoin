...
#if defined(NDEBUG)
# error "Lowlycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xea22ca928d9e8712ffdfff591374fbec7b91ec44522b31cc4b004389a06171f8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Lowlycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Lowlycoin Signed Message:\n";
...
{
    // Lowlycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000000); // Lowlycoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Lowlycoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Lowlycoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Lowlycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Lowlycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbedc9120485edc28a57055c9f087a60ac0d76fe0898aaffc691d19e880b02b9d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Koreas agree talks to defuse tension";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e146f008dac42ee7aa25a379269734cd67b2d58adb8ad2dc0794e478b921dc4067f6f7025ac7b741d9f60f769ecb35a862c2254cfcd2ca407e298dfd3f65faa0") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515514894;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085246299;

        if (fTestNet)
        {
            block.nTime    = 1515514383;
            block.nNonce   = 386226319;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x4a501b02b4722a2158bdd721eff5835079bd2becf71ea8670c6c55cb2cfce7ed"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc4, 0xbd, 0xd5 }; // Lowlycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LowlycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LowlycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LowlycoinMiner(CWallet *pwallet)
{
    printf("LowlycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lowlycoin-miner");
    ...
    try { loop {
        ...
        printf("Running LowlycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LowlycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LowlycoinMiner, pwallet));
}
